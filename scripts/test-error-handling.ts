...
async function testValidHtmlGeneration() {
  ...
  try {
    ...
    logTest('Valid HTML generates PDF successfully', false, (error as Error).message);
  }
}

// Test 2: Invalid HTML - Malformed structure
async function testInvalidHtmlMalformed() {
  console.log('\n📋 Test 2: Invalid HTML - Malformed structure');
  try {
    const invalidHtml = `
      <!DOCTYPE html>
    ...
    // If we get here, Puppeteer handled the malformed HTML gracefully
    logTest('Malformed HTML error handling', true, 'Puppeteer handled malformed HTML gracefully');
  } catch (error) {
    // Expected to catch error
    const errorMessage = (error as Error).message;
    if (errorMessage.includes('Failed to generate PDF')) {
      logTest('Malformed HTML error handling', true, 'Caught expected error');
    } else {
      logTest('Malformed HTML error handling', false, `Unexpected error: ${errorMessage}`);
    }
  }
}

// Test 3: Invalid HTML - Empty content
...
// Test 4: Invalid HTML - Extremely large content
async function testInvalidHtmlLarge() {
  console.log('\n📋 Test 4: Invalid HTML - Extremely large content');
  try {
    // Generate a very large HTML document
    const largeContent = '<p>' + 'Lorem ipsum dolor sit amet. '.repeat(100000) + '</p>';
    const largeHtml = `
      <!DOCTYPE html>
      <html>
    ...
    if (result && result.length > 0) {
      logTest('Large HTML handling', true, `Generated in ${duration}ms`);
      console.log(`  PDF size: ${result.length} characters (base64)`);
    } else {
      logTest('Large HTML handling', false, 'Empty result');
    }
  } catch (error) {
    const errorMessage = (error as Error).message;
    logTest('Large HTML handling', false, errorMessage);
  }
}

// Test 5: Invalid HTML - Script injection attempt
async function testInvalidHtmlScript() {
  console.log('\n📋 Test 5: Invalid HTML - Script injection');
  try {
    const scriptHtml = `
      <!DOCTYPE html>
      <html>
      ...
      <body>
        <h1>Test</h1>
        <script>
          // This should not execute in PDF generation
    ...
    const result = await generatePdf({
      htmlContent: scriptHtml,
      resumeData: mockResumeData
    });

    if (result && result.length > 0) {
      logTest('Script injection handling', true, 'Scripts safely ignored in PDF');
    } else {
      logTest('Script injection handling', false, 'Empty result');
    }
  } catch (error) {
    const errorMessage = (error as Error).message;
    if (errorMessage.includes('Script executed')) {
      logTest('Script injection handling', false, 'Script was executed (security issue)');
    } else {
      logTest('Script injection handling', true, 'Error caught safely');
    }
  }
}
...
// Test 7: Browser cleanup on error
async function testBrowserCleanup() {
  ...
  try {
    ...
    logTest('Browser cleanup on error', false, (error as Error).message);
  }
}

// Test 8: Performance - Generation time
async function testGenerationPerformance() {
  console.log('\n📋 Test 8: Generation performance');
  try {
    const validHtml = `
      <!DOCTYPE html>
      <html>
    ...
    logTest('Generation performance', false, (error as Error).message);
  }
}
...
async function runAllTests() {
  ...
  await testInvalidHtmlMalformed();
  await testInvalidHtmlEmpty();
  await testInvalidHtmlLarge();
  await testInvalidHtmlScript();
